...
import static com.sscp.core.impl.models.link.LinkImpl.*;
    ...
    public Pair<Page, String> resolveRedirects(@Nullable final Page page) {
        ...
        if (page != null && page.getPageManager() != null) {
            ...
            while (result != null && StringUtils
                    .isNotEmpty((redirectTarget = result.getProperties().get(NameConstants.PN_REDIRECT_TARGET, String.class)))) {
                result = page.getPageManager().getPage(redirectTarget);
                if (result != null) {
                    if (!redirectCandidates.add(result.getPath())) {
                        log.warn("Detected redirect loop for the following pages: {}.", redirectCandidates);
                        break;
                    }
                }
            }
