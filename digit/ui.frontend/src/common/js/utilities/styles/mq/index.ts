...
export const getZeroMediaQuery = (): string | undefined => {
...
const mq = (bp, includeMedia = true): string => `${includeMedia ? '@media ' : ''}(min-width: ${BREAKPOINTS[bp]}px)`;
...
mq.between = (bpA, bpB, includeMedia = true): string =>
  `${includeMedia ? '@media ' : ''}(min-width: ${BREAKPOINTS[bpA]}px) and (max-width: ${BREAKPOINTS[bpB] - 1}px)`;
