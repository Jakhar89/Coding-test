...
describe('utils/props/generateHelper', () => {
  ...
  it('it calls optional applyValue parameter', () => {
    ...
    expect(applyValue).toHaveBeenNthCalledWith(1, supportedProps, theme, 'mx', '10px', BP_KEY_0, undefined);
    expect(applyValue).toHaveBeenNthCalledWith(2, supportedProps, theme, 'mx', '20px', BP_KEY_1, undefined);
  });
...
describe('utils/props/generateHelper.defaultGetCssString', () => {
  ...
  it('returns correct output non-zero breakpoint', () => {
    ...
    expect(css).toEqual(`@media (min-width: ${BP_VAL_1}px) { margin-top: 10px; margin-bottom: 10px; }`);
  });
});
