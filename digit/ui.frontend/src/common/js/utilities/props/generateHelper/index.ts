import { isPlainValue, isObject, IPropList } from '../types';
...
export interface IHelperOptions {
  ...
  applyValue?: (SUPPORTED_PROPS: IPropList, theme: any, property: string, value?: any, breakpoint?: string) => string[];
  getCssString?: (css: string | string[], breakpoint?: string | undefined) => string;
}
/**
...
const defaultGetValue = (props: any, property: string, theme?: any) => (props ? props[property] : undefined);
...
const defaultApplyValue = (
  ...
  breakpoint?: string,
  config?: any,
): string[] => SUPPORTED_PROPS[property].map((prop) => `${prop}: ${value};`);
...
export default (SUPPORTED_PROPS: IPropList, { theme, config = undefined, ...rest }, options?: IHelperOptions) =>
  Object.keys(rest)
    .filter((key) => Object.keys(SUPPORTED_PROPS).indexOf(key) > -1)
    .reduce((prev: any, property) => {
      ...
      if (isPlainValue(value)) {
        const css: string[] = applyValue(SUPPORTED_PROPS, theme, property, value, undefined, config);
      ...
      return prev.concat(
        ...mediaQueries.map((breakpoint) => {
          const css: string[] = applyValue(SUPPORTED_PROPS, theme, property, value[breakpoint], breakpoint, config);
