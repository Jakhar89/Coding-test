...
const restoreOverflowSetting = () => {
  ...
  setTimeout(() => {
    if (previousBodyPaddingRight !== 'undefined') {
      document.body.style.width = null; // previousBodyPaddingRight;
...
export const disableBodyScroll = (targetElement, options: any = {}) => {
  if (isIosDevice) {
    ...
    elements.forEach(element => {
      if (element && !locks.some((lock) => lock.targetElement === element)) {
        const lock = {
...
export const clearAllBodyScrollLocks = () => {
  if (isIosDevice) {
    ...
    if (documentListenerAdded) {
      // cast here because typescript doesn't like passive
...
export const enableBodyScroll = (targetElement) => {
  ...
  if (isIosDevice) {
    ...
    elements.forEach(element => {
      ...
      locks = locks.filter((lock) => lock.targetElement !== element);
    });
